...
class Ephemerid:
    ...
    def sunrise(self, when=None) -> time:
        """
        ...
        """
        if when is None:
            when = datetime.now(tz=LOCAL_TIMEZONE)
        self.__preptime(when)
        self.__calc()
        return Ephemerid.__timefromdecimalday(self.sunrise_t)

    def sunset(self, when=None) -> time:
        if when is None:
            when = datetime.now(tz=LOCAL_TIMEZONE)
        self.__preptime(when)
        self.__calc()
        return Ephemerid.__timefromdecimalday(self.sunset_t)

    def solarnoon(self, when=None) -> time:
        if when is None:
            when = datetime.now(tz=LOCAL_TIMEZONE)
        self.__preptime(when)
        self.__calc()
        return Ephemerid.__timefromdecimalday(self.solarnoon_t)

    @staticmethod
    def __timefromdecimalday(day) -> time:
        """
...
if __name__ == "__main__":
    s = Ephemerid(lat=48.21, long=16.36)
